    ...
    namespace Audio
    {
        public partial class Pach_SP_HRTF
        {
            ...
            public static double[][] ResampleHRIRWDL(float[][] hrir, int srcFs, int targetFs)
            {
                ...
                // Flatten channels into double[channels][samples]
                double[][] multiChannel = new double[channels][];
                for (int ch = 0; ch < channels; ch++)
                {
                    float[] temp = Pach_SP.Resample(hrir[ch], srcFs, targetFs, 0, false);
                    multiChannel[ch] = new double[temp.Length];
                    for(int i = 0; i < temp.Length; i++)
                        multiChannel[ch][i] = temp[i];
                }
                return multiChannel;
            }
        ...
        {
            public class SystemCompensationSettings
            {
